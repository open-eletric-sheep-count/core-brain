#!/usr/bin/env bash
#
# check.sh — validate @oesc/context-inject in place (single-folder plugin).
#
# This directory IS the distribution artifact (global/opencode/plugins/
# context-inject/): there is no copy/build step; check.sh only proves the
# plugin is sound before install/publish.
#
# Steps:
#   1. Import smoke check — node imports index.ts (native TS type-stripping;
#      executes module top-level only, setup is NOT called).
